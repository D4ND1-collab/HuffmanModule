...
public final class FrequencyTable {
    ...
    public CodeTree buildCodeTree() {
        ...
        for (int i = 0; i < frequencies.length; i++) {
            if (frequencies[i] > 0)
                pqueue.add(new NodeWithFrequency(new LeafNode(i), i, frequencies[i]));
        }

        for (int i = 0; i < frequencies.length && pqueue.size() < 2; i++) {
            if (frequencies[i] == 0)
                pqueue.add(new NodeWithFrequency(new LeafNode(i), i, 0));
        }
        if (pqueue.size() < 2)
        ...
        while (pqueue.size() > 1) {
            ...
            pqueue.add(new NodeWithFrequency(
                    new ClassicNode(x.node, y.node),
                    Math.min(x.lowestSymbol, y.lowestSymbol),
                    x.frequency + y.frequency));
        }

        return new CodeTree((ClassicNode) pqueue.remove().node, frequencies.length);
    }
